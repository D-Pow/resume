...
\par
Git, \ React, \ MobX, \ Spring Boot, \ Regex, \ Webpack, \ Bash, \ Bootstrap, \ Jest, \ Gradle, \ JUnit, \ Maven, \ SQLite, \ Oracle DB, \ Linux,
\par
...
\ul{
    \item Pioneered the company's first Android Edge panel widget that displayed a user's watch lists and respective stock data.

    \item Orchestrated the state management, authentication, and caching systems; Coordinated UI/UX design and testing (JUnit).

    \item Facilitated a seamless hand-off of MVP to maintainers by writing extensive documentation and applying good source control practices.
}
...
\textbf{Eagle Scout} (2011): Highest rank, top 2\% of all Boy Scouts.
\\
\textbf{Cyber Security Club} (Sept. 2016 -- May 2017).

\columnbreak

\textbf{NYU Healthcare Hackathon Winner} (Nov. 2016).
\\
\textbf{Japanese} (2019 -- Present): Approaching N5 fluency.
